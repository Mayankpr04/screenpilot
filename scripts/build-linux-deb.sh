...
mkdir -p "$root/DEBIAN" "$root/usr/bin" "$root/usr/share/applications" \
  "$root/usr/share/icons/hicolor/scalable/apps" "$root/lib/udev/rules.d" "$output"
...
install -m 0644 native-linux/packaging/60-screenpilot-i2c.rules \
  "$root/lib/udev/rules.d/60-screenpilot-i2c.rules"
